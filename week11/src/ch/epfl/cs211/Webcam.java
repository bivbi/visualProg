...
public class Webcam extends PApplet {
					...
					for (int j = 0; j < Kernel[0].length; j++) {
						...
						int clampedY = (y + j - 1 < 0) ? 0
								: ((y + j - 1 > img.height - 1) ? img.height - 1
										: y + j - 1);
						reds += red(img.pixels[clampedX + img.width * clampedY])
								* Kernel[i][j];
						greens += green(img.pixels[clampedX + img.width
								* clampedY])
								* Kernel[i][j];
						blues += blue(img.pixels[clampedX + img.width
								* clampedY])
								* Kernel[i][j];
					}
				}
				result.pixels[x + img.width * y] = color(reds / weight, greens
						/ weight, blues / weight); // divisé par 9.0 ??? Ferait
	...
	public PImage sobel(PImage img) {
		...
		img.filter(GRAY); // Do SOBEL only on greyscale images !!! In order to
							// brigtness() not te return the value of BLUE
							// channel !!!
		...
		for (int x = 0; x < img.width; x++) {
			for (int y = 0; y < img.height; y++) {
				...
				// TODO: sum_h et sum_v divisées par weight ??? A quoi sert ???
				// --> Moyenne pondérée, donc divisé par 9 ??? (taille du
				// Kernel)
		...
		for (int y = 2; y < img.height - 2; y++) { // Skip top and bottom edges
			for (int x = 2; x < img.width - 2; x++) { // Skip left and right
				if (buffer[y * img.width + x] > (int) (max * 0.15f)) { // 15% of
																		// the
																		// max
